...
class SynthesisLayerWrapper(torch.nn.Module, LoRALayer):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        # x = modulated_conv2d(x=x, weight=(self.synthesis_layer.weight + (self.lora_B @ self.lora_A).view(self.synthesis_layer.out_channels, self.synthesis_layer.kernel_size, self.synthesis_layer.in_channels, self.synthesis_layer.kernel_size).permute(0, 2, 1, 3) * self.scaling), styles=styles, noise=noise, up=self.synthesis_layer.up, padding=self.synthesis_layer.padding, resample_filter=self.synthesis_layer.resample_filter, flip_weight=flip_weight, fused_modconv=fused_modconv)
