...
def _conv2d_gradfix(transpose, weight_shape, stride, padding, output_padding, dilation, groups):
    ...
    class Conv2d(torch.autograd.Function):
        ...
        def forward(ctx, input, weight, bias):
            ...
            ctx.save_for_backward(
                input if weight.requires_grad else _null_tensor.to(weight.device),
                weight if input.requires_grad else _null_tensor.to(input.device),
            )
    ...
    class Conv2dGradWeight(torch.autograd.Function):
        ...
        def forward(ctx, grad_output, input, weight):
            ...
            ctx.save_for_backward(
                grad_output if input.requires_grad else _null_tensor.to(input.device),
                input if grad_output.requires_grad else _null_tensor.to(grad_output.device),
            )
